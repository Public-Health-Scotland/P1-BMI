...
hb_res_file <- haven::read_sav(paste0(output_folder, "P1BMIData.zsav")) %>%
  group_by(school_year, year_ending, HB_RESIDENCE_DESC, simd, sex) %>%
...
all_part_hb_file <- haven::read_sav(paste0(output_folder, "P1BMIData.zsav")) %>%
  group_by(school_year, year_ending, HB_RESIDENCE_DESC, COUNCIL_AREA_DESC, simd, sex) %>%
  summarise_at(vars(N_Valid_Height_Weight:Clin_Obese_SevObese), sum) %>%
...
p1_bmi_agg_file <- rbind(hb_res_file, la_file, all_part_hb_file) %>%
...
haven::write_sav(completeness_data, paste0(FinalOutput, "P1_Board_coverage_data.sav"), compress = FALSE)